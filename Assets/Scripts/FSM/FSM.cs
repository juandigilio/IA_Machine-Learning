    ...
    where FlagType : Enum
{
    ...
    public void ValidateParameters(Type[] expectedParameters, Func<object[]> recivedParameters)
    {
        ...
        {
            throw new ArgumentException($"Expected {expectedParameters.Length} parameters, but received {recivedParametersTypes.Count}.");
        }
        ...
        for (int i = 0; i < expectedParameters.Length; i++)
        {
            ...
            {
                throw new ArgumentException("Type " + recivedParametersTypes[i].Name + " cannot be asigned to " + expectedParameters[i].Name);
            }
        }
    }
}